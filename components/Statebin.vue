    ...
    <div class="statebinContainer">
        ...
        <div class="statebins">
            <div :style="'top:' + bin.y + 'px;left:' + bin.x + 'px;background-color:' + bin.color" class="statebin" v-for="bin in bins" v-tooltip="{ content: '<b>' + bin.name + '</b><br>' + Math.round(bin.percent*100) + ' percent<br>' + bin.number + ' workers' }">
                <!-- <span style="z-index: 200"> -->{{bin.abbrev}}<!-- </span> -->
                <!-- <div class="binPercent" :style="'background-color:black;position: absolute; bottom:0; left:0; width:100%; z-index: 100; height:' + Math.round(bin.percent*100) + '%'"></div> -->
            </div>
        </div>
...
export default {
    ...
    data() {
        return {
            stats: (this.race === 'governors' ? governors : legislators).map(row => {
                return {
                    state: row.state,
                    percent: +row[(this.race === 'governors' ? 'pct_governor/lieutenant_governor_out_of_state' : 'pct_state_house/assembly/senate_out_of_state')],
                    number: +row[(this.race === 'governors' ? 'contributions_18_governor/lieutenant_governor_out_of_state' : 'contributions_18_state_house/assembly/senate_out_of_state')]
                };
            }),
            grid: [
                '                                  ME',
    ...
    computed: {
        bins() {
            ...
            this.stats.forEach(function(d) {
                let abbrev = d.state; // postal(d.state);
